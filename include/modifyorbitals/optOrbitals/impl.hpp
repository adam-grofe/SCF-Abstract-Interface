...
void OptimizeOrbitals<MatsT>::runModifyOrbitals(EMPerturbation& pert, std::vector<std::reference_wrapper<SquareMatrix<MatsT>>> mo, std::vector<double*> eps) {
  ...
  for( size_t a=0; a<den.size(); a++)
    prevOnePDM[a] = *den[a];
  scfConv.prevEnergy = this->modOrbOpt.getTotalEnergy();
...
template <typename MatsT>
void OptimizeOrbitals<MatsT>::printRunHeader(std::ostream& out, EMPerturbation& pert) {
...
template <typename MatsT>
void OptimizeOrbitals<MatsT> :: printHeaderFinal(std::ostream& out) {
 ...
 */
template <typename MatsT>
void OptimizeOrbitals<MatsT>::printIteration(std::ostream& out, bool printDiff) {
...
bool OptimizeOrbitals<MatsT>::evalProgress(EMPerturbation& pert) {
  ...
  if( MPIRank(this->comm) == 0 ) {
    ...
    this->modOrbOpt.computeProperties(pert);
    this->scfConv.deltaEnergy = this->modOrbOpt.getTotalEnergy() - scfConv.prevEnergy;
    scfConv.prevEnergy = this->modOrbOpt.getTotalEnergy();
...
double OptimizeOrbitals<MatsT> :: computeDensityConv() {
    std::vector<std::shared_ptr<SquareMatrix<MatsT>>> currDen = this->modOrbOpt.getOnePDM();
