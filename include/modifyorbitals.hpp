...
struct ModifyOrbitalsOptions {
    //
    //--------------------------------------------------------------------------------------------
    // REQUIRED VARIABLES FOR SOME ALGORITHMS
    std::vector<size_t> nOcc;   ///< Number of occupied orbitals in each set of MO's
    std::vector<int> nC;        ///< Number of Components
    ...
    std::function<std::vector<std::shared_ptr<SquareMatrix<MatsT>>>()>
    ...
    std::function<std::vector<std::shared_ptr<Orthogonalization<MatsT>>>()>
...
class ModifyOrbitals {
  ...
  public:
    ...
    virtual void runModifyOrbitals(EMPerturbation& pert, std::vector<std::reference_wrapper<SquareMatrix<MatsT>>>, std::vector<double*>) = 0;

    // getNewOrbitals performs only a single step in the
    // optimization/simulation
    virtual void getNewOrbitals(EMPerturbation& pert, std::vector<std::reference_wrapper<SquareMatrix<MatsT>>>, std::vector<double*>) = 0;

    // Printing functions
    virtual void printRunHeader(std::ostream& out, EMPerturbation&)                   = 0;
    virtual void printIteration(std::ostream& out = std::cout, bool printDiff = true) = 0;
};
